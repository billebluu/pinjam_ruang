...
$user = query($data);
$data_laporan = query("SELECT * FROM data_laporan WHERE id=$id")[0];
// ambil id dari URL
...
if (isset($_POST["submit"])) {
    ...
    if (ubahLaporan($_POST) > 0) {
        echo "<script>
            ...
            window.location.href = 'admin-data-laporan-pinjam.php';
            </script>";
    } else {
        echo "<script>
                    ...
                    <div class="col-auto col-sm-9 p-5 min-vh-100 d-flex flex-column justify-content-between" style="margin-left:320px; border-radius: 0 50px 50px 0;">
                        <div>
                            <h5 class="admin-text"><strong>Admin</strong></h5>
                                <div class="data-text">
                                    <h1><strong>Laporan Peminjaman Ruang</strong></h1>
                                    <br>
                                        ...
                                        <section>
                                            <div class="container-profile" align="left">
                                                <form class="row g-3" action="" method="post" enctype="multipart/form-data">
                                                    ...
                                                    <input type="hidden" name="sik" value="<?= $data_laporan["sik"]; ?>">
                                                    <div class="col-md-5">
                                                        <label for="nama_pengaju" class="form-label">Nama Pengaju</label>
                                                        <input type="text" class="form-control" name="nama_pengaju"  id="nama_pengaju" value="<?= $data_laporan["nama_pengaju"]; ?>" disabled>
                                                    </div>
                                                    <div class="col-md-5">
                                                        <label for="nama_ruang" class="form-label">Ruang</label>
                                                        <input type="text" class="form-control" id="nama_ruang"  name="nama_ruang" value="<?= $data_laporan["nama_ruang"]; ?>" disabled>
                                                    </div>
                                                    <div class="col-md-5">
                                                        <label for="email" class="form-label">Email</label>
                                                        <input type="text" class="form-control" id="email"  name="email" value="<?= $data_laporan["email"]; ?>" disabled>
                                                    </div>
                                                    <div class="col-md-5">
                                                        <label for="kegiatan" class="form-label">Kegiatan</label>
                                                        <input type="text" class="form-control" id="kegiatan"  name="kegiatan" value="<?= $data_laporan["kegiatan"]; ?>" disabled>
                                                    </div>
                                                    <div class="col-md-5">
                                                        <label for="gender" class="form-label">Jenis Kelamin</label>
                                                        <input type="text" class="form-control" name="gender"  id="nama_pengaju" value="<?= $data_laporan["gender"]; ?>" disabled>
                                                    </div>
                                                    <div class="col-md-5">
                                                        <label for="statusUser" class="form-label">Status</label>
                                                        <input type="text" class="form-control" name="statusUser"  id="statusUser" value="<?= $data_laporan["statusUser"]; ?>" disabled>
                                                    </div>
                                                    <div class="col-md-5">
                                                        <label for="phone" class="form-label">No. Telephone</label>
                                                        <input type="text" class="form-control" id="phone"  name="phone" value="<?= $data_laporan["phone"]; ?>" disabled>
                                                    </div>
                                                    <div class="col-md-5">
                                                        <label for="tanggal" class="form-label">Tanggal</label> 
                                                        <input type="date" class="form-control" id="tanggal"  name="tanggal" value="<?= $data_laporan["tanggal"]; ?>" disabled>
                                                    </div>
                                                    <div class="col-md-5">
                                                        <label for="nim_nip" class="form-label">NIM/NIP</label>
                                                        <input type="text" class="form-control" id="nim_nip"  name="nim_nip" value="<?= $data_laporan["nim_nip"]; ?>" disabled>
                                                    </div>
                                                    <div class="col-md-5">
                                                        <label for="waktu_awal" class="form-label">Waktu Awal</label>
                                                        <input type="time" class="form-control" id="waktu_awal"  name="waktu_awal" value="<?= date("H:i", strtotime($data_laporan["waktu_awal"])); ?>" disabled>
                                                    </div>
                                                    <div class="col-md-5">
                                                        <label for="waktu_akhir" class="form-label">Waktu Akhir</label>
                                                        <input type="time" class="form-control" id="waktu_akhir"  name="waktu_akhir" value="<?= date("H:i", strtotime($data_laporan["waktu_akhir"])); ?>" disabled>
                                                    </div><br><br><br><br><br>
                                                    <div class="col-md-5"></div>
                                                    <div class="col-md-5">
                                                        <label for="sik">Berkas</label><br><br>
                                                        <button class="button-action" style="background-color: rgb(201,239,255);"> <a class="nav-link" href="<?= $data_laporan["ktm"]; ?>">Lihat KTM</a></button>
                                                        <button class="button-action" style="background-color: rgb(201,239,255);"> <a class="nav-link" href="<?= $data_laporan["sik"]; ?>">Lihat SIK</a></button>
                                                </form>
                                            </div>
                                          </section>
                                        </div>
